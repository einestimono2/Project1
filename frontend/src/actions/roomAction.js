...
export const roomStatistics =
  (address = "Toàn quốc", time = [], status = "Đã phê duyệt") =>
  async (dispatch) => {
    try {
      dispatch({
      ...
      let link = `/api/admin/rooms/statistic?status=${status}`;
      ...
      if (time[0] && time[1]) {
        link += `&createdAt[gte]=${time[0].toISOString()}&createdAt[lte]=${time[1].toISOString()}`;
      }
