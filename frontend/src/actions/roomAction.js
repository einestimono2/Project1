...
import {
  ...
  ADMIN_ROOM_FAIL,
  //
  STATISTIC_ROOM_REQUEST,
  STATISTIC_ROOM_SUCCESS,
  STATISTIC_ROOM_FAIL,
  //
...
export const statisticRooms =
  (category = "Tất cả", time = "Tất cả", status = "Đã phê duyệt") =>
  async (dispatch) => {
    try {
      dispatch({
        type: STATISTIC_ROOM_REQUEST,
      });

      let link = `/api/admin/rooms?status=${status}`;

      if (category !== "Tất cả") link += `&category=${category}`;

      if (time !== "Tất cả") {
        switch (time) {
          case "Hôm nay":
            const today = getBeginningOfTheDay();
            link += `&createdAt[gte]=${today}`;
            break;

          case "Tuần này":
            const firstDayOfWeek = getBeginningOfTheWeek();
            link += `&createdAt[gte]=${firstDayOfWeek}`;
            break;

          case "Tháng này":
            const firstDayOfMonth = getBeginningOfTheMonth();
            link += `&createdAt[gte]=${firstDayOfMonth}`;
            break;

          case "Năm nay":
            const firstDayOfYear = getBeginningOfTheYear();
            link += `&createdAt[gte]=${firstDayOfYear}`;
            break;
      ...
      dispatch({
        type: STATISTIC_ROOM_SUCCESS,
        payload: data.rooms,
      });
    } catch (error) {
      dispatch({
        type: STATISTIC_ROOM_FAIL,
        payload: error.response.data.message,
      });
    }
  };
...
export const myRooms = () => async (dispatch) => {
  try {
    dispatch({ type: MY_ROOMS_REQUEST });

    const { data } = await axios.get("/api/myrooms");

    dispatch({ type: MY_ROOMS_SUCCESS, payload: data.rooms });
  } catch (error) {
    dispatch({
      type: MY_ROOMS_FAIL,
      payload: error.response.data.message,
