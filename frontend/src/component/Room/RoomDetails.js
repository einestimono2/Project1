...
const RoomDetails = () => {
  ...
  const bookingHandler = () => {
    if (user && user._id === room.user?._id) {
      ...
      return;
    } else if (room.numOfRooms === 0) {
      alert.error("Đã hết phòng!");
      return;
    } else {
      ...
      {loading ? (
        ...
        <Fragment>
          ...
          >
            ...
            <Link color="inherit" to="/rooms">
              Danh sách phòng
            </Link>
            <Link to={`/room/${room?._id}`}>#{room?._id}</Link>
          </Breadcrumbs>
          ...
          <div className="containerRoom">
            <div className="roomHeader">
              <a rel="nofollow" href={`/room/${room._id}`}>
                {room.name}
              </a>
            ...
            <div className="recentRooms">
              ...
              {rooms &&
                rooms
                  .filter((e) => e._id !== room._id)
                  .reverse()
                  .slice(0, 15)
                  .map((room) => <RoomTile key={room._id} room={room} />)}
            </div>
          </div>
